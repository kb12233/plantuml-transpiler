  ...
  generateClass(classObj, classDiagram) {
    ...
    if (classObj.constructors.length > 0) {
      for (const constructor of classObj.constructors) {
        ...
        // Parameters
        code += constructor.parameters.map(param => `${this.mapCSharpType(param.type)} ${param.name}`).join(', ');
        code += ')\n' + this.indent('{\n', 2);
    ...
    for (const method of classObj.methods) {
      ...
      // Parameters
      code += method.parameters.map(param => `${this.mapCSharpType(param.type)} ${param.name}`).join(', ');
      code += ')';
